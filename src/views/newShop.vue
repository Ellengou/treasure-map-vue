            ...
            <el-row :gutter="20">
              <el-col :span="10">
                <div class="grid-content">
                  ...
                  <div class="grid-box" style="min-height: 1400px;">
                    <el-form-item label="店铺名称" prop="name" required>
                      <el-input v-model="formProduct.name" style="width: 500px;"></el-input>
                    </el-form-item>
                    <el-form-item label="所属性质" prop="curioCityId" required>
                      <el-select v-model="formProduct.curioCityId" placeholder="请选择" style="width: 500px;">
                        <el-option label="独立店铺" :value="-1"></el-option>
                        <el-option
                    ...
                    <el-form-item label="经营范围" prop="tagIds">
                      <el-checkbox-group v-model="formProduct.tagIds">
                        <el-checkbox v-for="item in tagData" :label="String(item.id)">{{item.name}}</el-checkbox>
                      </el-checkbox-group>
                      <el-input v-model="formProduct.scope" placeholder="其它经营范围" style="width: 500px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="营业执照" prop="businessList" required>
                      <el-upload
                        :action="basePath+'/image/upload'"
                        type="drag"
                        :thumbnail-mode="true"
                        ...
                        :default-file-list="formProduct.businessList">
                        <i class="el-icon-upload"></i>
                        <div class="el-dragger__text">将文件拖到此处，或<em>点击上传</em></div>
                        <div class="el-upload__tip" slot="tip">只能上传图片文件，且不超过1M</div>
                      </el-upload>
              ...
              <el-col :span="14">
                <div class="grid-content">
                  <div class="grid-top">其它信息</div>
                  <div class="grid-box" style="min-height: 1400px;">
                    <div class="grid-photo">
                      <div v-for="file in formProduct.shopList" class="photo">
                        <img :src="file.url">
                        <div class="el-upload__file is-finished" style="margin: 5px 0px;">
                          <a class="el-upload__file__name">
                            <i class="el-icon-document"></i>{{file.name}}
                          </a>
                          <span class="el-upload__btn-delete" @click="handleRemove2(file)">删除</span>
                        </div>
                      </div>
                    </div>
                    <el-form-item label="门店照片">
                      <img src="../../static/bg.png" width="200" height="200" style="margin-bottom: 10px;"
                           v-if="!formProduct.shopList.length">
                      <el-upload
                        :action="basePath+'/image/upload'"
                        :multiple="true"
                        :before-upload="uploadBefore"
                        ...
                        <div class="el-upload__tip" slot="tip" style="margin-top: 5px;">
                          仅支持JPG、JPEG、PNG格式的图片文件，文件不能大于1MB, 上传数量不能大于9张
                        </div>
                      </el-upload>
                    </el-form-item>
...
<script>
  ...
  export default {
    mounted(){
      ...
      if (id) {
        this.axios.get('/api/shop/' + id).then((response) => {
          if (response.data.code == 0) {
            this.id = response.data.result.model.id;
            this.formProduct.name = response.data.result.model.name;
            this.formProduct.curioCityId = response.data.result.model.curioCityId;
            this.formProduct.tagIds = _.uniq(response.data.result.model.tagIds);
            ...
            this.formProduct.businessList = [{name: '默认', url: response.data.result.model.shopLicense}];
            this.formProduct.shopList = response.data.result.model.shopView;
    ...
    data(){
      return {
        ...
        rules: {
          ...
          curioCityId: [
            {required: true, type: 'number', message: '请选择所属性质', trigger: 'change'}
          ],
          tagIds: [
    ...
    methods: {
          ...
          if (valid) {
            let curio = {
              param: {
                ...
                curioCityId: this.formProduct.curioCityId,
                tagIds: _.uniq(this.formProduct.tagIds),
                ...
                cid: {name: this.formProduct.businessList[0].name, url: this.formProduct.businessList[0].url},
                pics: this.formProduct.shopList,
            ...
            this.axios.post('/api/shop/add-update', JSON.stringify(curio)).then((response) => {
              if (response.data.code == 0) {
                let string = this.$route.query.id ? '店铺编辑成功' : '店铺新增成功';
      ...
      handleRemove1(file, fileList) {
        ...
        this.formProduct.businessList = _.difference(this.formProduct.businessList, [file])
      },
      ...
      handleRemove2(file, fileList) {
        ...
        this.formProduct.shopList = _.difference(this.formProduct.shopList, [file])
      },
      handleSuccess2(response, file, fileList){
        if (this.formProduct.shopList.length < 9) {
          this.formProduct.shopList = _.concat(this.formProduct.shopList, {
            name: file.name,
            url: response.result.model
          });
        }
