      ...
      <div class="display">
        <div class="display-main">
          <el-form label-position="top" :model="formLabel" :rules="rules" ref="formLabel">
            <el-form-item label="标签" prop="name" required>
      ...
      handleSubmit() {
        this.$refs.formLabel.validate((valid) => {
          if (valid) {
            ...
            this.axios.post('/api/basic/tag/add-update', JSON.stringify(label)).then((response) => {
              if (response.data.code == 0) {
                let string = this.$route.query.id ? '标签编辑成功！': '标签新增成功！';
                this.$message.success({
                  message: string,
                  showClose: true
                });
                this.$router.replace({path: '/label'})
              } else {
                this.$message.error({
