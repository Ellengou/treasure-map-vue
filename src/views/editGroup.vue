...
<script>
  export default {
    mounted(){
      ...
      if (id) {
        this.axios.get('/api/basic/account/' + id).then((response) => {
          if (response.data.code == 0) {
            this.formGroup.id = response.data.result.model.id;
            this.formGroup.username = response.data.result.model.username;
            this.formGroup.roleIds = parseInt(response.data.result.model.roleIds);
          }
        })
      }
    },
